...
add_repo(){
    sudo add-apt-repository -y $2 >>setup.log 2>&1 && echo "  ${GREEN} Repo $1 added${NC}"
}

slow_install(){
    for i in $1; do
    apt-get install -q -y $i >>/dev/null 2>&1 && echo "  ${GREEN} installed $i${NC}" | tee setup.log || echo "*** ${RED}$i not installed${NC} ***" | tee setup.log
    done
}

fast_install(){
    for i in $1; do
       sudo apt-fast install -q -y $i >>/dev/nul 2>&1  && echo " ${GREEN}  installed $i${NC}" | tee setup.log || echo "*** ${RED}$i not installed${NC} ***" | tee setup.log
    done
}
...
special_apps="tlp variety telegram neofetch tor-browser youtube-dl atom sublime-text docker heroku spotify-client numix-icon-theme-circle numix-folders arc-theme tor-browser docker-engine indicator-notifications-gtk2"
...
#repos
    ...
    add_repo indicator-notifications  ppa:jconti/recent-notifications
...
echo "special repo installs"
    sudo apt-get -y -q install $special_apps >>setup.log 2>&1  || slow_install $special_apps

echo "now for the software..."
    sudo apt-fast -y install $package_list || fast_install $package_list
